...
from formutils import getAppRequestDataTable, getAppRequestDataTableTeamInfo
...
class TeamMail(EMail):
    '''
    This class is special to the application shot. It handles all emails to team members.
    '''
    def __init__(self, auth, tid, slug_base, account_id = 'team', mass = False):
        EMail.__init__(self, account_id, mass)
        self.tid = tid
        self.auth = auth
        self.db = auth.db
        self.teammember = self.db.auth_user(tid)

        self.html = current.response.render('email/email_base.html', dict(body = self.auth.get_shotwiki_page(slug_base)))

        self.receiver = self.teammember['email']

        self.subs['<PLACEHOLDER_NAME>'] = self.teammember.first_name
...
class AppropriationRequestMail(ShotMail):
    ...
    def __init__(self, auth, aid):
        ...
        self.subs['<PLACEHOLDER_APPROPRIATION_REQUEST>']  = getAppRequestDataTable(ar_row)

class AppropriationRequestTeamInfoMail(TeamMail):
    '''
    This class defines the email sent to the team members to inform about a new appropriation request.
    '''
    def __init__(self, auth, tid, aid):
        TeamMail.__init__(self, auth, tid, 'email-teaminfo-request')

        ar_row = auth.db.request(aid)

        self.subject = 'Neuer Förderantrag: %s' % ar_row.project
        self.send_backup    = False
        self.subs['<PLACEHOLDER_APPROPRIATION_REQUEST>'] = getAppRequestDataTableTeamInfo(ar_row)
        self.subs['<PLACEHOLDER_REQUEST_URL>'] = config.shoturl + 'staff/crud/request/view?id=%d' % aid
