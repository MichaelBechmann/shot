...
class PersonEntry():
    ...
    def __init__(self, db, data, pid = None):
        ...
        # The database field 'mail_enabled' is boolean. The form variable is of type string ('yes' / 'no') because it shall be displayed as radio buttons.
        # If the 'mail_enabled' information is not part of the data do nothing to not overwrite existing database entries.
...
class Numbers():
    ...
    def number_of_available_not_reserved(self):
        '''
        This method returns the number of available sale numbers which shall not be reserved for a former helper.
        '''
...
class Contributions():
    ...
    def get_shifts(self, scope = None):
        ...
        scope: This argument allows to restrict the result to a single scope. If left away the scope attribute is ignored.
        '''
...
class WaitList():
    ...
    def status_text(self, pid):
        ...
        # determine or predict position on the wait list
        pos = self.get_pos_current(pid)
        if pos == 0:
            pos = self.length() + 1

        numbers = Numbers(self.db, self.eid)
        x  = numbers.number_of_available() - n_open_contributions - pos
        ...
        if x > 15*numbers.limit/100: # > 15% of all numbers still free
            ...
            if b_contributions_available:
                msg = 'Es sind noch einige Kommissionsnummern frei. Sie können sich gerne auf die Warteliste setzen. Ob Sie dann eine Nummer erhalten, hängt davon ab, wie viele Helfer und Kuchenspender wir noch gewinnen können. Um ganz sicher eine Nummer zu erhalten, tragen Sie sich für eine Helferschicht oder einen Kuchen ein.'
            else:
            ...
            if b_contributions_available:
                msg = 'Es sind derzeit Kommissionsnummern nur noch für Helfer und Kuchenspender verfügbar. Sie können sich aber gerne auf die Warteliste setzen. Sollten Nummern zurückgegeben werden, könnten Sie evtl. noch eine erhalten, ohne am Markt zu helfen oder Kuchen zu spenden.'
            else:
...
class User():
    ...
    def get_email_subscriptions(self):
        query  = self.db.auth_email_subscription.user_id == self.uid
        query &= self.db.auth_email_subscription.email_type_id == self.db.auth_email_type.id

        return self.db(query).select(orderby = self.db.auth_email_type.id)

class Team():
    '''
    This class contains code related to the team members.
    '''
    def __init__(self, db):
        self.db = db
    ...
    def get_all_members_for_info_email(self):
        '''
        This method generates a list of all team members who have email information configured.
        Currently there exists only one type of information email (info on new appropriation requests). Once there are more this function shall get a parameter specifying the info type.

        This method returns a rows object containing the IDs and names of the team members in the auth_user table.
        '''
        query  = self.db.auth_user.registration_key == ''
        query &= self.db.auth_user.id == self.db.auth_email_subscription.user_id
        query &= self.db.auth_email_subscription.email_type_id == self.db.auth_email_type.id
        query &= self.db.auth_email_type.name == 'info_on_new_request'

        rows = self.db(query).select(self.db.auth_user.id, self.db.auth_user.last_name, self.db.auth_user.first_name)
        rows.compact = False
        return rows
